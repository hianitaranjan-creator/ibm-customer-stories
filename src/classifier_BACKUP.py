...
def classify_story(story: dict) -> dict:
    ...
    # GTM motions for the story (union across all proof points).
    story_gtm: set[str] = set()

    # Classify each proof point.
    strengths = []
    for proof in story.get("proof_points", []):
    ...
    # Roll up to story level.
    story["proof_strength"] = _highest_strength(strengths)
